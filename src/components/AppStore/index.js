...
class AppStore extends Component {
  state = {category: tabsList[0].tabId, input1: ''}
  ...
  updateValue = event => {
    ...
    this.setState({input1: value1})
  }

  filterData = () => {
    const {input1, category} = this.state

    const resInput = input1.toLowerCase()

    const filteredList = appsList.filter(eachItem => {
      const resName = eachItem.appName.toLowerCase()
      return resName.includes(resInput) && eachItem.category === category
    })
          ...
          <div className="cardsContainer">
            <ul className="cards">
              {filteredList.map(eachItem => (
                <Card cardInfo={eachItem} key={eachItem.appId} />
              ))}
            </ul>
          </div>
        </div>
